...
The power analysis for this study demonstrated that to observe a large true effect (~10% difference between treatments), our sample size should be over 500, and to observe a small true effect (~3% difference between treatments), our sample size should be over 2500. Our study involved 672 individuals providing us the power to observe a large true effect. 
...
These scale of these tabular results can be visualized using a sankey diagram. 
...
sankey_df <- rbind(sankey_df, sankey_df_2)
...
links <- data.frame(source = c(0, 0, 0,1, 1,1, 2, 2,2, 3, 3, 4, 4, 5, 5),
                    ...
                    value = sankey_df$total_responses)


sn <- sankeyNetwork(Links = links, Nodes = nodes, Source = "source", 
                    Target = "target", Value = "value", NodeID = "id", 
                    NodeGroup = "group", fontSize = 14, nodeWidth = 30)

# Save the HTML widget as a temporary file
tmp_file <- tempfile(fileext = ".html")
htmlwidgets::saveWidget(sn, tmp_file)

# Take a screenshot of the HTML widget and save it as an image file
webshot(tmp_file, "sankey.png", vwidth = 1200, vheight = 800, zoom = 2, delay = 1)
```

```{r, out.width = "100%", fig.cap = "Insert Caption"}
knitr::include_graphics("sankey.png")
```

Upon analyzing the relationship between ticket resolution time and survey response rates, we observed an inconsistent pattern. Response rates are 44% for same-day resolutions, slightly lower at 42.5% for one-day resolutions, and 42% for two-day resolutions. Interestingly, the response rate significantly increases to 53% for three-day resolutions. This suggests that complex factors influence the relationship, warranting further investigation. We further examine the distribution of ticket response times. 
...
stargazer(m1_robust, m2_robust, m3_robust, type = 'latex')
```
\newpage
